...
$(document).ready(function(){
	...
	function read_comment_include(){
		...
		var article = $("#read_form input[name='article']").val();
		$("._CALLING_COMMENT").load(__URL_PATH__+"modules/board/read.comment.inc.php?board_id="+board_id+"&read="+read+"&article="+article+"&viewDir=m/");
	}
	read_comment_include();
			...
			$("#read_comment_form .comment_modify").each(function(index){
				if($(this).attr("name")==$imgVar.attr("name")){
					if($(this).css("display")!="none"){
						...
						var boxWidth = $(".comment_form_box").width();
